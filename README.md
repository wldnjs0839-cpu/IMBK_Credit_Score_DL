...
### 2️. Type_of_Loan — Multi-hot Encoding
...
<img width="1317" height="528" alt="01_class_distribution" src="https://github.com/user-attachments/assets/62498867-bd2a-4fbe-9782-d50d71af5ad5" />
...
<img width="1187" height="708" alt="02_mutual_information" src="https://github.com/user-attachments/assets/7bfcdfcd-fb10-492c-91e6-7ee08cf46f82" />
...
<img width="1045" height="946" alt="03_correlation_heatmap" src="https://github.com/user-attachments/assets/7f886999-fbdd-4093-a5d0-0aefabf50ee7" />
...
<img width="1549" height="947" alt="04_features_vs_target" src="https://github.com/user-attachments/assets/130a3041-8b67-4848-afce-2b6dc6a2666d" />
...
<img width="1309" height="647" alt="05_loan_type_distribution" src="https://github.com/user-attachments/assets/428695a1-2e6a-47b9-84b9-f7044a8e8a4b" />
...
<img width="1429" height="528" alt="06_customer_panel" src="https://github.com/user-attachments/assets/632561fc-450a-419d-a591-1b6c356849dd" />
...
| 후보 | 채택 여부 | 사유 |
|---|---|---|
| **TabTransformer** | X | LabelEncoding 후 거의 수치형 → 어텐션 이득 미미 |
| **TabNet** | X | 구현 복잡도 대비 MLP와 큰 차이 없음 |
| **기본 MLP** (64→32→3) | X | 표현 용량 부족 (71%대 정체) |
| **MLP + Residual** | O | [512,256,128,64] + Skip Connection |
| **XGBoost** | O | 정형 데이터 강자. 단독 83%+ |
| **LightGBM** | O | XGB 보완용. 단독 82%+ |
...
... (Fold 5까지) ...         │     MLP 0.25 / XGB 0.45 / LGB 0.30
                             │
                             ↓
                       Final Prediction (83.70%)
```

**실제 결과 분석**:
- MLP 단독: 83.31% (K-Fold 평균 82.38%)
- XGBoost 단독: 83.45% (K-Fold 평균 82.79%)
- LightGBM 단독: 82.73% (K-Fold 평균 82.08%)
- 앙상블 최종: **83.70%**

→ MLP가 예상보다 강력하게 학습됨 (Customer 집계 피처의 효과)
→ 앙상블은 최고 단독 모델(XGB 83.45%) 대비 +0.25%p 추가 향상
...
<img width="230" height="645" alt="학습곡선" src="https://github.com/user-attachments/assets/bbfd01e4-c701-44c3-a198-ce00c87b6a33" />

Cosine 스케줄러로 학습률이 점진적으로 감소하면서 후반부 val accuracy의 진동이 줄어드는 모습이 관찰됩니다. Early Stopping이 patience=15로 동작해 과적합 전 자동 종료됩니다.

### Confusion Matrix

<img width="510" height="237" alt="리포트" src="https://github.com/user-attachments/assets/79b50781-61c2-45ff-b77b-f07db005f24f" />

대각선(정답) 비율이 높고, Standard ↔ Good / Standard ↔ Poor 간 일부 혼동이 보입니다. 인접한 등급 사이의 경계는 본질적으로 모호한 영역이라 자연스러운 패턴.

### 모델별 성능 비교

| 모델 | K-Fold CV 평균 | Hold-out Test |
|---|---|---|
| 기본 MLP (64→32→3, 베이스라인) | - | 71.52% |
| **개선 MLP + Residual** | 82.38% (±0.41%) | 83.31% |
| **XGBoost (5-fold 평균)** | 82.79% (±0.28%) | 83.45% |
| **LightGBM (5-fold 평균)** | 82.08% (±0.24%) | 82.73% |
| **3-모델 가중 앙상블 (최종)** | - | **83.70%** |
...
[Classification Report]
        ...
        Good       0.80      0.84      0.82      3566
        Poor       0.81      0.87      0.84      5799
    Standard       0.87      0.81      0.84     10635
...
| # | 개선 항목 | 기여도 |
|---|---|---|
| ① | **XGBoost + LightGBM 앙상블 도입** | +0.3~0.4%p (단독 대비) |
| ② | **Customer-level 집계 피처** (평균/표준편차 26개) | +4~5%p |
| ③ | **MLP + Residual 구조 개선** | +3~4%p |
| ④ | **5-Fold Cross Validation** | +0.5~1%p (안정성) |
| ⑤ | **Type_of_Loan Multi-hot Encoding** | +1~1.5%p |
| ⑥ | **학습 전략 개선** (AdamW + Cosine + Early Stop) | +1~1.5%p |
| ⑦ | **비율 피처 생성** (Debt/Income 등) | +0.5~1%p |
| ⑧ | **데이터 누수 차단** (fold 내부 scaler fit) | 누수 제거 |

**총 개선**: 71.52% → 83.70% (**+12.18%p**)

**주요 발견**: 
- 본 데이터에서는 **Customer-level 집계 피처**가 가장 큰 기여 (+4~5%p)
- MLP 단독(83.31%)도 XGBoost(83.45%)와 비슷한 성능 달성
- 앙상블은 단독 최고 모델 대비 추가 +0.25%p 개선 (83.45% → 83.70%)
- 트리 모델이 절대적 우위가 아닌, **피처 엔지니어링이 핵심**임을 확인
...
- **약한 데이터 누수**: Customer-level 집계는 같은 고객의 행이 train/val에 섞이면서 약한 정보 누출 여지가 있음. 실제 서비스 시나리오가 "신규 고객 예측"이라면 **GroupKFold**(Customer_ID 기준)로 분할 필요. 다만 본 프로젝트에서는 이 집계 피처가 가장 큰 성능 향상(+4~5%p)을 가져왔음.

- **앙상블 효과 제한적**: 실제 결과에서 앙상블(83.70%)이 최고 단독 모델(XGB 83.45%) 대비 +0.25%p만 추가 향상. 세 모델의 예측이 유사해 다양성이 부족한 것으로 추정. **Stacking(MetaLearner)** 또는 **CatBoost** 추가 시 다양성 확보 가능.

- **시계열 활용**: 본 프로젝트는 8개월의 통계량만 사용. **LSTM/Transformer**로 시계열 패턴 자체를 학습하면 추가 개선 가능.

- **하이퍼파라미터 자동 탐색**: 현재는 수동 튜닝. **Optuna**로 XGB/LGB의 max_depth, n_estimators 등을 자동 탐색하면 단일 모델 성능 추가 향상 가능.

- **클래스 불균형 대응**: Standard(53%) / Poor(29%) / Good(18%) 불균형에 대해 class_weight 미적용. **Focal Loss** 또는 **SMOTE** 적용 시 소수 클래스(Good) 성능 개선 가능.
